...
# Cargar los datos
int_random = pd.read_csv('bruteforcerand.csv')
real_random = pd.read_csv('bruteforcerealrand.csv')
identity = pd.read_csv('bruteforceident.csv')
constant = pd.read_csv('bruteforceconst.csv')
...
# Etiquetas y formatow
plt.xlabel('Tamaño de matriz (n)', fontsize=12)
plt.ylabel('Tiempo promedio (nanosegundos)', fontsize=12)
plt.title('Algoritmo de Fuerza Bruta - Comparación de tipos de matrices', fontsize=14)
plt.legend(loc='upper left', fontsize=10)
plt.grid(True, alpha=0.3, linestyle='--')

# Guardar
plt.tight_layout()
plt.savefig('bruteforce_plot.png', dpi=150)
plt.show()
